...
class RemoteCore
{
 public:
  ...
  std::shared_ptr<SolutionThree> getSolutionThree(const std::map<float, int>& circTypes, float precision);
 ...
 private:
  ...
  void handleTwo(const Aws::SQS::Model::Message& msg, const nlohmann::json& bodyJson);
  void handleThree(const Aws::SQS::Model::Message& msg, const nlohmann::json& bodyJson);
